...
class _ProductDetailViewState extends State<ProductDetailView> {
    ...
    return SliverAppBar(
      ...
      flexibleSpace: FlexibleSpaceBar(
        background: (product.imageUrls.isEmpty)
          ...
          child: const Icon(Icons.image_search_outlined, size: 80, color: Colors.grey),
        )
            : Stack(
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('${formatter.format(product.displayPrice)} / ${product.unit}', style: Theme.of(context).textTheme.headlineMedium?.copyWith(color: Theme.of(context).colorScheme.primary, fontWeight: FontWeight.w700)),
          ...
          if(product.attributes != null && product.attributes!.isNotEmpty)
            _buildInfoSection(context, title: 'Thông tin chi tiết', child: _buildAttributesTable(context, product.attributes!)),
        ],
      ),
    );
  }
  ...
  Widget _buildAttributesTable(BuildContext context, Map<String, dynamic> attributes) {
    return Table(columnWidths: const {0: IntrinsicColumnWidth(),1: FlexColumnWidth(),},children: attributes.entries.map((entry) {return TableRow(children: [Padding(padding: const EdgeInsets.symmetric(vertical: 8.0, horizontal: 8.0),child: Text('${entry.key}:',style: Theme.of(context).textTheme.bodyLarge?.copyWith(fontWeight: FontWeight.bold),),),Padding(padding: const EdgeInsets.symmetric(vertical: 8.0, horizontal: 8.0),child: Text(entry.value.toString(),style: Theme.of(context).textTheme.bodyLarge?.copyWith(color: Colors.black.withOpacity(0.7)),),),],);}).toList(),);
  }
