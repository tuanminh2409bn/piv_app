...
// --- HÀM HELPER CHO TÍNH NĂNG "MUA NGAY" ---
Future<void> _showBuyNowDialog(
  ...
  if (authState is! AuthAuthenticated) {
    ScaffoldMessenger.of(context)
        .showSnackBar(const SnackBar(content: Text('Vui lòng đăng nhập để mua hàng.')));
    return;
  }
  if (product.packingOptions.isEmpty) {
    ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Sản phẩm chưa có quy cách đóng gói.')));
    return;
  }
  ...
  showDialog<PackagingOptionModel>(
          ...
          return AlertDialog(
            ...
            content: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<PackagingOptionModel>(
                  ...
                  onChanged: (PackagingOptionModel? newValue) {
                    setState(() {
            ...
            actions: <Widget>[
              ...
              ElevatedButton(
                ...
                onPressed: () {
                  Navigator.of(dialogContext).pop(tempSelectedOption);
                },
              ),
            ],
          );
    ...
    if (selected != null) {
      final itemToBuy = CartItemModel(
        ...
        price: selected.getPriceForRole(userRole),
        itemUnitName: selected.unit,
        quantity: 1, // Mua ngay mặc định 1 thùng/đơn vị
        quantityPerPackage: selected.quantityPerPackage,
        caseUnitName: selected.name,
        // SỬA LỖI: Thêm categoryId để tính năng gợi ý hoạt động
...
class ProductDetailView extends StatelessWidget {
          ...
          if (cartState.status == CartStatus.success) {
            ...
            ScaffoldMessenger.of(context)
              ...
              ..showSnackBar(SnackBar(
                  content:
                  Text(cartState.errorMessage ?? 'Thêm vào giỏ hàng thất bại.'),
                  ...
                  slivers: <Widget>[
                    ...
                    SliverToBoxAdapter(
                      child: _buildProductInfo(
                          context, product, state.quantity, selectedOption, userRole),
                    ),
  ...
  Widget _buildProductInfo(
      ...
      ProductModel product,
      int quantity,
      PackagingOptionModel? selectedOption,
      String userRole) {
  ...
  Widget _buildPackagingSelector(BuildContext context,
      List<PackagingOptionModel> options, PackagingOptionModel? selectedOption) {
    return DropdownButtonFormField<PackagingOptionModel>(
      value: selectedOption,
      isExpanded: true,
      hint: const Text('Chưa có quy cách đóng gói'),
      decoration: InputDecoration(
        labelText: 'Chọn quy cách đóng gói',
        border: const OutlineInputBorder(),
        prefixIcon: const Icon(Icons.inventory_2_outlined),
        fillColor: options.isEmpty ? Colors.grey.shade200 : null,
        filled: options.isEmpty,
      ),
      onChanged: options.isEmpty
          ? null
          : (PackagingOptionModel? newValue) {
        if (newValue != null) {
          context
              .read<ProductDetailCubit>()
              .selectPackagingOption(newValue);
        }
      },
  ...
  Widget _buildQuantitySelector(BuildContext context, int quantity) {
    return Row(
      children: [
        ...
        GestureDetector(
          onTap: () => _showQuantityInputDialog(context, quantity),
          child: Container(
            width: 60,
          ...
          children: [
            ...
            Expanded(
              child: ElevatedButton(
                ...
                onPressed: () {
                  _showBuyNowDialog(context, product, userRole);
                },
              ),
            ),
          ],
  ...
  Widget _buildProductImageAppBar(BuildContext context, ProductModel product) { return SliverAppBar(expandedHeight: MediaQuery.of(context).size.width, stretch: true, pinned: true, elevation: 2.0, backgroundColor: Colors.white, leading: _buildAppBarActionIcon(child: IconButton(icon: const Icon(Icons.arrow_back, color: Colors.white),onPressed: () => Navigator.of(context).pop(),tooltip: 'Quay lại')), automaticallyImplyLeading: false, actions: [_buildAppBarActionIcon(child: IconButton(icon: const Icon(Icons.home_outlined, color: Colors.white),tooltip: 'Về Trang chủ',onPressed: () => Navigator.of(context).popUntil((route) => route.isFirst))),_buildAppBarActionIcon(child: CartIconWithBadge(iconColor: Colors.white, onPressed: () => Navigator.of(context).push(CartPage.route()))),const SizedBox(width: 8)], flexibleSpace: FlexibleSpaceBar(background: (product.imageUrl.isEmpty) ? Container(color: Colors.grey.shade200, child: const Icon(Icons.image_search_outlined, size: 80, color: Colors.grey)) : Image.network(product.imageUrl, fit: BoxFit.cover, width: double.infinity, errorBuilder: (context, error, stackTrace) => const Icon(Icons.broken_image_outlined, size: 80, color: Colors.grey)))); }
