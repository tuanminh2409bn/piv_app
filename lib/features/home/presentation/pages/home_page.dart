...
class HomeView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String userRole = 'agent_2'; // Mặc định
    if (authState is AuthAuthenticated) {
      userIdentifier = authState.user.displayName ?? authState.user.email ?? authState.user.id;
      userRole = authState.user.role; // Lấy vai trò của người dùng
    }
      ...
      itemBuilder: (context, index) {
        ...
        return ListTile(
          leading: (article.imageUrl.isNotEmpty)
              ? Image.network(article.imageUrl, width: 70, height: 70, fit: BoxFit.cover)
              : Container(width: 70, height: 70, color: Colors.grey.shade200, child: const Icon(Icons.article_outlined)),
          title: Text(article.title, style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w600)),
          subtitle: Text(
