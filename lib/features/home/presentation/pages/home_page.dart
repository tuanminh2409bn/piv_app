...
class HomeView extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // ... (code AppBar và BlocBuilder phần đầu không đổi) ...
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (state.featuredProducts.isNotEmpty)
                          _buildFeaturedProductsGrid(context, state.featuredProducts) // featuredProducts giờ là List<ProductModel thật>
                        else
...
// --- WIDGETS HELPER CHO HOMEVIEW ---
// _buildBannerCarousel, _buildSectionTitle, _buildCategoriesRow giữ nguyên
              ...
              onTap: () {
                ...
                if (targetUrl != null && targetUrl.isNotEmpty) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('Điều hướng tới: $targetUrl')),
                  );
                }
              },
              child: Container(
                ...
                child: ClipRRect(
                  borderRadius: BorderRadius.circular(12),
                  child: Image.network(
                    banner.imageUrl, // Sử dụng imageUrl từ BannerModel thật
                    fit: BoxFit.cover,
        ...
        return SizedBox(
          ...
          child: Card(
            ...
            child: InkWell(
              onTap: () {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('Mở danh mục: ${category.name}')),
                );
              },
...
// Widget cho lưới sản phẩm nổi bật (Sử dụng ProductModel thật)
Widget _buildFeaturedProductsGrid(BuildContext context, List<ProductModel> products) {
  ...
  return GridView.builder(
    ...
    itemBuilder: (context, index) {
      final product = products[index]; // product là ProductModel THẬT
      return Card(
        elevation: 3,
        clipBehavior: Clip.antiAlias,
        ...
        child: InkWell(
          onTap: () {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('Xem sản phẩm: ${product.name}')),
            );
          },
          child: Column(
            ...
            children: [
              Expanded(
                ...
                child: (product.imageUrls.isNotEmpty)
                    ? Image.network(
                  product.imageUrls.first, // Lấy ảnh đầu tiên từ list
                  fit: BoxFit.cover,
                  errorBuilder: (context, error, stackTrace) => Container(
                    color: Colors.grey.shade200, alignment: Alignment.center,
                    child: const Icon(Icons.image_search_outlined, size: 50, color: Colors.grey),
                  ),
                )
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                        currencyFormatter.format(product.displayPrice), // Định dạng giá tiền
...
// _buildNewsList và _buildPlaceholderContainer giữ nguyên vì chúng vẫn dùng model giả
Widget _buildNewsList(BuildContext context, List<NewsArticleModel> newsArticles) {
  return ListView.separated(
    ...
    itemBuilder: (context, index) {
      final article = newsArticles[index]; // article là NewsArticleModel giả
      return Card(
          ...
          leading: ClipRRect(
            ...
            child: Image.network(
              ...
              errorBuilder: (context, error, stackTrace) => Container(
                  ...
                  child: const Icon(Icons.article_outlined, color: Colors.grey, size: 30,)
              ),
            ),
          ),
          title: Text(article.title, style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w600), maxLines: 2, overflow: TextOverflow.ellipsis),
          subtitle: Text(article.summary, maxLines: 1, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.bodySmall),
          trailing: Icon(Icons.arrow_forward_ios, size: 16, color: Colors.grey.shade600),
          onTap: () {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('Đọc tin: ${article.title}')),
            );
          },
        ),
      );
