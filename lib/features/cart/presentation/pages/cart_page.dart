...
class CartView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: BlocListener<CartCubit, CartState>(
        // --- SỬA LỖI: Cập nhật Listener để lắng nghe các trạng thái thành công cụ thể ---
  ...
  Widget _buildCartItemCard(BuildContext context, CartItemModel item, NumberFormat formatter) {
    return Padding(
      ...
      child: InkWell(
        onTap: () {
          Navigator.of(context).push(ProductDetailPage.route(item.productId));
        },
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: [
              ClipRRect(borderRadius: BorderRadius.circular(8), child: Image.network(item.imageUrl, width: 80, height: 80, fit: BoxFit.cover, errorBuilder: (c,e,s) => Container(width:80, height:80, color: Colors.grey.shade200, child: const Icon(Icons.image)))),
              ...
              Column(
                ...
                children: [
                  IconButton(padding: EdgeInsets.zero, constraints: const BoxConstraints(), icon: Icon(Icons.delete_outline, color: Colors.red.shade400, size: 24), onPressed: () => context.read<CartCubit>().removeProduct(item.productId, item.caseUnitName)),
                  const SizedBox(height: 24),
                  Text(formatter.format(item.subtotal), style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                ],
              )
            ],
          ),
  ...
  Widget _buildSummarySection(BuildContext context, CartState state, NumberFormat formatter) { return Container(padding: const EdgeInsets.fromLTRB(16, 16, 16, 24), decoration: BoxDecoration(color: Theme.of(context).scaffoldBackgroundColor, boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.08), blurRadius: 10, offset: const Offset(0, -5))], border: Border(top: BorderSide(color: Colors.grey.shade200, width: 1.5))), child: Column(children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text('Tổng cộng (${state.uniqueItemCount} loại sản phẩm):', style: Theme.of(context).textTheme.titleMedium), Text(formatter.format(state.totalPrice), style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.primary))]), const SizedBox(height: 16), SizedBox(width: double.infinity, child: ElevatedButton(onPressed: state.items.isEmpty ? null : () => Navigator.of(context).push(CheckoutPage.route()), child: const Text('Tiến hành Thanh toán')))])); }
}